...
class LicensePlateView:
    def __init__(self, root):
        ...
        # Thiết lập font chữ và bảng màu
        ...
        self.photo = None  # Lưu ảnh để tránh bị xóa bởi garbage collection

    def create_main_layout(self):
        """Tạo bố cục chính với lưới 12 cột"""
        ...
        self.main_container.rowconfigure(1, weight=1)  # Nội dung chính
        self.main_container.rowconfigure(2, weight=0)  # Bảng điều khiển

    def setup_header(self):
        """Tạo phần tiêu đề với logo và tên hệ thống"""
        ...
        self.logo_label = ctk.CTkLabel(self.header, text="🚗", font=("Roboto", 30), text_color="#ffffff")
        self.logo_label.pack(side="left", padx=(20, 0))
        
        self.title_label = ctk.CTkLabel(self.header, text="HỆ THỐNG NHẬN DIỆN BIỂN SỐ XE", font=("Roboto", 22, "bold"), text_color="#ffffff")
        self.title_label.pack(side="left", padx=15)

    def setup_sidebar(self):
        """Tạo sidebar chứa lịch sử nhận diện với bố cục tối ưu"""
        self.sidebar = ctk.CTkFrame(self.main_container, fg_color=self.card_bg, corner_radius=15, border_width=1, border_color=self.card_border)
        self.sidebar.grid(row=1, column=0, columnspan=3, sticky="nsew", padx=(20, 10), pady=20)

        # Tiêu đề và số lượng lịch sử
        self.history_frame = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        self.history_frame.pack(fill="x", padx=10, pady=(10, 5))

        self.history_label = ctk.CTkLabel(self.history_frame, text="📜 Lịch Sử Nhận Diện", font=self.subheader_font, anchor="w")
        self.history_label.pack(side="left")

        self.history_count = ctk.CTkLabel(self.history_frame, text="0", font=self.small_font, width=25, height=25, corner_radius=12, fg_color=self.primary_color, text_color="#ffffff")
        self.history_count.pack(side="right", padx=(0, 5))

        # Ô tìm kiếm và nút làm mới
        ...
        self.search_entry = ctk.CTkEntry(self.search_frame, placeholder_text="Tìm kiếm biển số hoặc tên file...", font=self.normal_font, border_width=0, textvariable=self.search_var, height=35)
        self.search_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))

        self.refresh_btn = ctk.CTkButton(self.search_frame, text="↻", font=self.normal_font, fg_color=self.secondary_color, text_color="#ffffff", corner_radius=5, width=35, height=35, command=lambda: self.search_var.set(""))
        self.refresh_btn.pack(side="right")

        # Hai tab Hình Ảnh và Video
        self.tab_frame = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        self.tab_frame.pack(fill="x", padx=10, pady=(5, 0))

        self.tab_images = ctk.CTkButton(self.tab_frame, text="Hình Ảnh", font=self.normal_font, fg_color=self.primary_color, text_color="#ffffff", corner_radius=5, height=35)
        self.tab_images.pack(side="left", fill="x", expand=True, padx=(0, 5))

        self.tab_videos = ctk.CTkButton(self.tab_frame, text="Video", font=self.normal_font, fg_color="#e0e0e0", text_color=self.text_color, corner_radius=5, height=35)
        self.tab_videos.pack(side="left", fill="x", expand=True, padx=(5, 0))

        # Danh sách file (Treeview)
        ...
        style.configure("Treeview", background="#ffffff", foreground=self.text_color, rowheight=35, fieldbackground="#ffffff", borderwidth=0)
        style.map('Treeview', background=[('selected', self.primary_color)], foreground=[('selected', 'white')])
        style.configure("Treeview.Heading", background=self.bg_color, foreground=self.text_color, relief="flat", font=self.normal_font)

        self.image_scrollbar = ttk.Scrollbar(self.image_list_frame)
        self.image_scrollbar.pack(side="right", fill="y")

        self.image_treeview = ttk.Treeview(self.image_list_frame, columns=("filename", "plate"), show="headings", yscrollcommand=self.image_scrollbar.set)
        self.image_treeview.heading("filename", text="Tên File")
        self.image_treeview.heading("plate", text="Biển Số")
        self.image_treeview.column("filename", width=140)
        self.image_treeview.column("plate", width=90)
        ...
        self.video_treeview = ttk.Treeview(self.video_list_frame, columns=("date", "duration"), show="headings", yscrollcommand=self.video_scrollbar.set)
        self.video_treeview.heading("date", text="Ngày")
        self.video_treeview.heading("duration", text="Thời lượng")
        self.video_treeview.column("date", width=90)
        self.video_treeview.column("duration", width=140)
        self.video_treeview.pack(fill="both", expand=True)
        self.video_scrollbar.config(command=self.video_treeview.yview)

    def setup_content_area(self):
        """Tạo khu vực nội dung với canvas và thông tin chi tiết"""
        ...
        self.preview_card = ctk.CTkFrame(self.content_area, fg_color=self.card_bg, corner_radius=15, border_width=1, border_color=self.card_border)
        ...
        self.preview_title = ctk.CTkLabel(self.preview_header, text="Xem Trước", font=self.subheader_font, anchor="w")
        self.preview_title.pack(side="left")
        
        self.status_indicator = ctk.CTkLabel(self.preview_header, text="⬤ Sẵn sàng", font=self.small_font, text_color=self.accent_color)
        ...
        self.canvas = tk.Canvas(self.canvas_frame, bg="#f0f0f0", bd=0, highlightthickness=0)
        ...
        self.plate_container = ctk.CTkFrame(self.results_frame, fg_color=self.bg_color, corner_radius=8, height=60)
        self.plate_container.pack(fill="x", pady=10)
        
        self.plate_icon = ctk.CTkLabel(self.plate_container, text="🚘", font=("Roboto", 22))
        self.plate_icon.pack(side="left", padx=(15, 0))
        
        self.plate_label = ctk.CTkLabel(self.plate_container, text="Chưa có dữ liệu", font=("Roboto", 24, "bold"))
        ...
        self.time_frame = ctk.CTkFrame(self.details_container, fg_color=self.bg_color, corner_radius=8, height=40, width=200)
        self.time_frame.pack(side="left", fill="y", expand=True, padx=(0, 5))
        
        self.time_icon = ctk.CTkLabel(self.time_frame, text="🕒", font=("Roboto", 14))
        self.time_icon.pack(side="left", padx=(15, 0))
        
        self.time_label = ctk.CTkLabel(self.time_frame, text="Thời gian: --:--:--", font=self.normal_font)
        self.time_label.pack(side="left", padx=10)
        
        self.location_frame = ctk.CTkFrame(self.details_container, fg_color=self.bg_color, corner_radius=8, height=40, width=200)
        self.location_frame.pack(side="left", fill="y", expand=True, padx=5)
        
        self.location_icon = ctk.CTkLabel(self.location_frame, text="📍", font=("Roboto", 14))
        self.location_icon.pack(side="left", padx=(15, 0))
        
        self.location_label = ctk.CTkLabel(self.location_frame, text="Vị trí: Không xác định", font=self.normal_font)
        self.location_label.pack(side="left", padx=10)
        
        self.delete_frame = ctk.CTkFrame(self.details_container, fg_color=self.bg_color, corner_radius=8, height=40, width=200)
        self.delete_frame.pack(side="left", fill="y", expand=True, padx=(5, 0))
        
        self.delete_button = ctk.CTkButton(self.delete_frame, text="Xóa Lịch Sử", font=self.normal_font, fg_color="transparent", text_color="#000000", corner_radius=8, height=40, width=120, command=self.on_delete_history)
        self.delete_button.pack(side="left", padx=15)

    def setup_control_panel(self):
        """Tạo bảng điều khiển với nút chức năng và cài đặt IP"""
        self.control_panel = ctk.CTkFrame(self.main_container, fg_color=self.card_bg, corner_radius=15, border_width=1, border_color=self.card_border)
        ...
        self.realtime_btn = ctk.CTkButton(self.button_frame, text="Chế độ Real-time", font=self.normal_font, fg_color=self.accent_color, hover_color="#388e3c", corner_radius=8, height=40, width=180)
        self.realtime_btn.pack(side="left", padx=(0, 10))
        
        self.upload_btn = ctk.CTkButton(self.button_frame, text="Tải Ảnh/Video", font=self.normal_font, fg_color=self.primary_color, hover_color=self.secondary_color, corner_radius=8, height=40, width=180)
        self.upload_btn.pack(side="left", padx=10)
        
        self.export_btn = ctk.CTkButton(self.button_frame, text="Xuất Báo Cáo", font=self.normal_font, fg_color="#9e9e9e", hover_color="#757575", corner_radius=8, height=40, width=180)
        ...
        self.ip_label = ctk.CTkLabel(self.settings_frame, text="IP DroidCam:", font=self.normal_font)
        self.ip_label.pack(side="left", padx=(0, 10))
        
        self.ip_entry = ctk.CTkEntry(self.settings_frame, width=250, height=40, font=self.normal_font, corner_radius=8, placeholder_text="http://192.168.1.5:4747/video")
        self.ip_entry.insert(0, "http://192.168.1.5:4747/video")
        self.ip_entry.pack(side="left", padx=(0, 10))
        
        self.update_ip_btn = ctk.CTkButton(self.settings_frame, text="Cập Nhật IP", font=self.normal_font, fg_color=self.primary_color, hover_color=self.secondary_color, corner_radius=8, height=40, width=120)
        self.update_ip_btn.pack(side="left")

    def update_canvas(self, frame):
        """Cập nhật canvas với frame mới"""
    ...
    def update_file_lists(self, image_items, video_items, total_count):
        """Cập nhật danh sách ảnh và video"""
    ...
    def switch_tab(self, tab_name):
        """Chuyển đổi tab giữa Hình Ảnh và Video"""
    ...
    def set_delete_handler(self, handler):
        """Gắn handler cho hành động xóa lịch sử"""
